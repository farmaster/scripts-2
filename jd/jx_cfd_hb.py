...
def get_cfd100url():
    ...
    try:
        ...
        # jxcfd2_exchange_hb_202204
    except KeyError:
        print('获取最新url失败，可能是cookie已过期')
        send('财富岛抢购通知','cookie已失效')
        return
    else:      
        dwLv =resp['hongbao'][0]['dwLvl']
    return f'https://m.jingxi.com/jxbfd/user/ExchangePrize?strZone=jxbfd&dwType=3&dwLvl={dwLv}&ddwPaperMoney=100000&strPoolName={pool}&sceneval=2&g_login_type=1'
...
def cfd_qq(def_start_time):
    ...
    res = requests.get(cfd_url, headers=headers).text
    t2 = time.time()
    # 进行json转换
    data = json.loads(res)
    msg = data['sErrMsg']
